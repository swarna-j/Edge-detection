...
/* Xilinx Software Command Line Tool (XSCT) Commands for downloading memory to file
...
uint32_t inputImg_hw[IMAGE_SIZE];
...
int main()
{
    ...
    {
    	inputImg_hw[i] = img[j];
    	inputImg_hw[i] = (inputImg_hw[i]<<8)|(img[j+1]);
    	inputImg_hw[i] = (inputImg_hw[i]<<8)|img[j+2];
    	j = j+3;
    }
